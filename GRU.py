...
def getCallbacks():
    log_dir=data_management.getModelPath(NAME, GRU_SIZE, LABEL_TYPE, TARGET_LENGTH, STEP_SIZE) #+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    return [
...
try:
    os.makedirs(data_management.getModelPath(NAME, GRU_SIZE, LABEL_TYPE, TARGET_LENGTH, STEP_SIZE))
...
if TRAIN:
    ...
    model.add(tf.keras.layers.GRU(GRU_SIZE, input_shape=(None, x_train.shape[-1])))
...
if PRETRAINED or not TRAIN:
    print('Loaded ' + data_management.getModelPath(NAME, GRU_SIZE, LABEL_TYPE, TARGET_LENGTH, STEP_SIZE))
    model = tf.keras.models.load_model(data_management.getModelPath(NAME, GRU_SIZE, LABEL_TYPE, TARGET_LENGTH, STEP_SIZE))
...
if TRAIN:
    ...
    # save model
    model.save(data_management.getModelPath(NAME, GRU_SIZE, LABEL_TYPE, TARGET_LENGTH, STEP_SIZE))
    
    # show train history
    visualizer.plot_train_history(history, NAME + ' ' + LABEL_TYPE + ' ' + str(GRU_SIZE))
